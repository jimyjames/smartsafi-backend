...
@booking_router.get("/admin/all")
def bookings_analytics(db: Session = Depends(get_db)):
    bookings = (
        db.query(Booking)
        .options(
            joinedload(Booking.client),
    ...
    for booking in bookings:
        ...
        response.append({
            "id": getattr(booking, "public_id", booking.id),

            # Client
            "customerId": getattr(client, "public_id", client.id) if client else None,
            "customerName": (
                f"{client.first_name} {client.last_name}"
                if client and client.first_name
                else getattr(client, "organization_name", None)
            ),

            # Worker
            "cleanerId": getattr(worker, "public_id", worker.id) if worker else None,
            "cleanerName": (
                f"{worker.first_name} {worker.last_name}"
                if worker and worker.worker_type == "individual"
                else getattr(worker, "organization_name", None)
            ),

            # Booking info
            "status": booking.status,
            "scheduledDate": (
                booking.appointment_datetime.date().isoformat()
